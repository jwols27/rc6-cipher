...
function encryptBlockRC6(plainBlock, expandedKey) {
...
function decryptBlockRC6(cipherBlock, expandedKey) {
...
function encrypt(plainData, expandedKey) {
...
function decrypt(cipherData, expandedKey) {
...
function encryptMessage() {
    ...
    document.getElementById("cipher").textContent = lastCipherHex.match(/.{1,32}/g)?.join("\n") ?? "";
    document.getElementById("plain").textContent = "";
    document.getElementById("cipher-input").value = lastCipherHex;
}
;
document.getElementById("encrypt-button")?.addEventListener("click", encryptMessage);
function decryptMessage() {
    if (!lastCipherHex) {
        alert("Criptografe primeiro.");
        return;
    }
    try {
        const S = expandKey();
...
function decryptHexInput() {
    const hexInput = document.getElementById("cipher-input").value.trim();
    if (!hexInput) {
        return;
    }
    try {
        ...
        if (bytes.length % BLOCK_SIZE !== 0) {
            alert("Texto criptografado deve corresponder a um múltiplo de 16.");
            return;
        }
        const S = expandKey();
